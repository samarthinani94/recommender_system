...
def load_glove(filename):
    ...
    for l in flines:
        wordnvec = l.split()
        glove[wordnvec[0]] = np.array(wordnvec[1:],dtype=float)
...
def filelist(root):
    ...
    allfiles = [f for f in allfiles if f.endswith('.txt')]
    return allfiles
...
def load_articles(articles_dirname, gloves):
    ...
    for fname in allfiles:
        filename_start = fname.find('/bbc/') + 5
        filename = fname[filename_start:]
        s = get_text(fname)

        text = s
        words_in_text = words(text)
        words_in_text = [w for w in words_in_text if w in gloves]
        centroid = sum([gloves[w] for w in words_in_text]) / len(words_in_text)

        s = s.split('\n')
        title = s[0]
        article = '\n'.join(s[1:])
        #centroid = doc2vec(article,gloves)
        articles_attr.append([filename,title,article,centroid])
    
    return articles_attr
...
def doc2vec(text, gloves):
    ...
    """
    words_in_text = words(text)
    words_in_text = [w for w in words_in_text if w in gloves.key]
...
def recommended(article, articles, n):
    ...
    """
    dist_art = distances(article, articles)
    ...
    #creating a list of distances sorted in ascending order
    sorted_keys = sorted(list(d.keys()))

    recommended = list()
    counter = 0
    for k in sorted_keys:
        recommended.append(d[k])
        counter += 1
        if counter == n+1:
            break
